...
/**
  ...
  */
class HourOfDay private (val value: Int) extends Ordered[HourOfDay] with Serializable {

  require(
    value >= HourOfDay.MIN && value <= HourOfDay.MAX,
    "Illegal value for 24 hour: %d , please use a value between 0 and 23"
      .format(value)
  )
...
object HourOfDay {
  ...
  def convertTo24hour(hour: Int, amPm: String): Int = {
    ...
    require(
      hour >= MIN && hour <= 12,
      "Illegal value for 12 hour: %d, please use a value between 0 and 11"
        .format(hour)
    )
