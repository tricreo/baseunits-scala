...
object CalendarDateTime {
  ...
  /**
    ...
    */
  def apply(aDate: CalendarDate, aTime: TimeOfDay): CalendarDateTime =
    from(aDate, aTime)
  ...
  /**
    ...
    *                                  引数`minute`が0〜59の範囲ではない場合もしくは、引数`day`が`yearMonth`の月に存在しない場合
    */
  def from(aDate: CalendarDate, aTime: TimeOfDay): CalendarDateTime =
    new CalendarDateTime(aDate, aTime)
