...
object TimeOfDay {
  ...
  /**
    ...
    */
  def apply(hour: HourOfDay, minute: MinuteOfHour): TimeOfDay =
  ...
  /**
    ...
    */
  def unapply(timeOfDay: TimeOfDay): Option[(HourOfDay, MinuteOfHour)] =
    Some(timeOfDay.hour, timeOfDay.minute)
  ...
  /**
    ...
    * @throws IllegalArgumentException 引数`hour`が0〜23の範囲ではない場合もしくは、引数`minute`が0〜59の範囲ではない場合
    */
  def from(hour: HourOfDay, minute: MinuteOfHour): TimeOfDay =
    apply(hour, minute)
  ...
  /**
    ...
    * @throws IllegalArgumentException 引数`hour`が0〜23の範囲ではない場合もしくは、引数`minute`が0〜59の範囲ではない場合
    */
  def from(hour: Int, minute: Int): TimeOfDay =
    new TimeOfDay(HourOfDay(hour), MinuteOfHour(minute))
