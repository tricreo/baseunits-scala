...
class HolidayCalendarExample extends Assertions with ShouldMatchers {
  ...
  def testDeriveBirthday {
    ...
    mlkBirthdays.next should be === CalendarDate.from(1929, 1, 15)
    mlkBirthdays.next should be === CalendarDate.from(1930, 1, 15)
    // etc.
    // By the way, to calculate how long MLK lived,
    mlkLifetime.length should be === Duration.days(14325)
  }
  ...
  def testDeriveThanksgiving {
    ...
    thanksgiving.isSatisfiedBy(CalendarDate.from(2005, 11, 24)) should be === true
    thanksgiving.isSatisfiedBy(CalendarDate.from(2005, 11, 25)) should be === false
    // Derive the date(s) for an interval
    thanksgiving.firstOccurrenceIn(CalendarInterval.year(2005)).get should be === CalendarDate.from(2005, 11, 24)

    // Calculate all the Thanksgivings over a three year interval.
    val y2002_2004 = CalendarInterval.inclusive(2002, 1, 1, 2004, 12, 31)
    thanksgiving.firstOccurrenceIn(y2002_2004).get should be === CalendarDate.from(2002, 11, 28)

    val iterator = thanksgiving.iterateOver(y2002_2004)
    iterator.next should be === CalendarDate.from(2002, 11, 28)
    iterator.next should be === CalendarDate.from(2003, 11, 27)
    iterator.next should be === CalendarDate.from(2004, 11, 25)
